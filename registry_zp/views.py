...
class MainView(IsAuthenticatedMixin, IsActiveUserMixin, TemplateView):
    template_name = 'registry_zp/main.html'
    ...
    def get(self, request, *args, **kwargs):
        ...
        SaveUserActivityStatistics.with_data(
            ...
            addition_data=f'Пользователь {request.user.username} просмтривает {RegistryNameChoices.REGISTRY_ZP.label} ({RegistryNameChoices.REGISTRY_ZP}).'
        )

        return self.render_to_response(context)


class RegistryZPViewSet(IsAuthenticatedMixin, IsActiveUserMixin, mixins.ListModelMixin, GenericViewSet):
    REGISTRY_INTERNAL_NAME = 'registry_zp'
