...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        # FILE_FOLDER = os.path.join(settings.DATA_DIR, r'source_files\registry_br_part_files')
        FILE_FOLDER = os.path.join(r'C:\OPEKA\ОУСВ №3\Реестр безработных')
        APP_BACKUP_FOLDER = os.path.join(settings.BACKUP_FOLDER, APP_NAME)
        # FILE_NAME = 'Безработные 1 янв-7 сент 2022.xlsx'
        ...
        # Выберем текущую таблицу для записи
        current_model = WorkWithStatistic.get_current_model_obj(REGISTRY_CHOICE_MARK)
        ...
        print(f'Всего строк добавлено в таблицу: {row_counter - 1 - EXCLUDE_COUNT_OF_LINES_FROM_FILE}')
        ...
        # Сделаем бэкап файла
        backup_file_name = f"{time.time()}_{file_obj.get_file_name}_part_file"
        try:
            shutil.copy2(
                file_obj.file_path,
