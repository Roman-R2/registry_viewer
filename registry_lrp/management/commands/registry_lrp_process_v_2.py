...
class Command(BaseCommand):
    help = 'Команда для парсинга реестра лищеных родительских прав'
    ...
    def __process_parsing(self, *args, **options):
        ...
        try:
            ...
            with open(TEMP_FILE_EXTRACTION_DIR / extract_file_name, mode='r') as csv_file:
                ...
                for line in tqdm(tuple(csv_file)):
                    ...
                    try:
                        ...
                        current_error = f'Произошла ошибка парсинга: \nline: {line} \nprocessed_line: ' \
                    ...
                    try:
                        ...
                        current_error = f'Ошибка добавления в DTO: {self.CURRENT_DTO.__name__}. split_line: {split_line}.' \
            ...
            LOGGER.info(
                f"Сохранили данные статистики об окончании парсинга {file_for_processing_dto.file_path.name}. {SaveParseStatistic.__name__}: id {save_parse_statistic_obj.id}"
            )
